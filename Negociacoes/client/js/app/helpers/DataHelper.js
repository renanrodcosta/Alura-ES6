...
var DataHelper = function () {
    ...
    _createClass(DataHelper, null, [{
        ...
        value: function textoParaData(texto) {
            ...
            var formatoCorreto = !/\d{2}\/\d{2}\/\d{4}/.test(texto);

            if (formatoCorreto) throw new Error('Deve estar no formato dd/mm/aaaa');

            return new (Function.prototype.bind.apply(Date, [null].concat(_toConsumableArray(texto.split("/").reverse().map(function (item, index) {
                return item - index % 2;
            })))))();
        }
    }]);
